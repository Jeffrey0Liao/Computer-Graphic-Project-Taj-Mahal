...
Water::Water(const int& gridX, const int& gridZ, const std::string& filename)
	: xGridDims(gridX), zGridDims(gridZ), frozen(false), time(0.0), radius(0.0)
{
...
void Water::Display()
{
	...
	glColor4f(1.0f, 1.0f, 1.0f, 0.4f);          // Set the base colour of the quad

	glPushMatrix();
	glTranslatef(pos[0], pos[1], pos[2]);   // position the water
	glScalef(scale[0], scale[1], scale[2]); // scale the unit water

	z = -0.5f; // initialise the start position as the top left coorner of the unit water mesh
	...
	for (int j = 0; j < zGridDims; j++)
	{
		...
		for (int i = 0; i < xGridDims; i++)
		{
			...
			glVertex3f(x, (float)sin(time + (double)x * 15) * radius + (float)sin(time + (double)z * 15) * radius, z);
			...
			glVertex3f(x, (float)sin(time + (double)x * 15) * radius + (float)sin(time + (double)(z + 1.0f / (float)zGridDims) * 15) * radius, z + 1.0f / (float)zGridDims);
			...
			glVertex3f(x + 1.0f / (float)xGridDims, (float)sin(time + (double)(x + 1.0f / (float)xGridDims) * 15) * radius + (float)sin(time + (double)(z + 1.0f / (float)zGridDims) * 15) * radius, z + 1.0f / (float)zGridDims);
			...
			glVertex3f(x + 1.0f / (float)xGridDims, (float)sin(time + (double)(x + 1.0f / (float)xGridDims) * 15) * radius + (float)sin(time + (double)z * 15) * radius, z);
